...
def prepare_data(eval):
    extract_path = f"{get_content_path()}/{eval}"
...
def setup_kiva_data_set(name, base_data_path):
    ...
    get_ipython().system(
        f"wget -q https://storage.googleapis.com/kiva-challenge/{zip_file_name} -O {download_zip_location}"
    )
    ...
    nested_path = os.path.join(
        target_img_dir, name
    )  # This would be like './data/train/train'
    ...
    get_ipython().system(
        f'wget -q -O {full_json_path} "https://storage.googleapis.com/kiva-key/{json_file_name}"'
    )
    ...
    stimuli_data = prepare_data(os.path.relpath(target_img_dir, get_content_path()))
...
def radar_plot_pt(
    ...
    fig, axs = plt.subplots(
        figsize=(8, 8), nrows=1, ncols=1, subplot_kw=dict(projection="radar")
    )
    ...
    for method, score in scores.items():
        if method in baselines:
            axs.plot(
                theta, score, color="red", linestyle="dashed", label=method
            )  # Red for random baseline (chance level = 1/3)
        else:
            axs.plot(
                theta, score, color="blue", label=method
            )  # Blue for 8-shot frequency
    ...
    axs.legend(prop={"size": 14}, loc="upper right", bbox_to_anchor=(1.3, 1.0))
...
def plot_tags(
    ...
    tags: Dict[str, Any],
    title: str,
    save_file: Any = None,  # Added title parameter
    width: float = 0.8,
    offset: float = 0.0,
) -> None:
    ...
    for idx, label in enumerate(labels):
        ax.bar(
            idx - offset, plot_values[idx], color="#6495ED", width=width
        )  # Set bars to blue
    ...
    plt.axhline(
        y=0.33, color="black", linestyle="dashed", label="Random Level (33%)"
    )  # Black for random baseline
...
def save_json_results(username, eval, output_folder, randomized_id, answer, image):
    ...
    # Append new result
    data.append({"id": randomized_id, "answer": answer})
...
def save_csv_results(
    id, model_response, full_model_response, original_json_path, csv_out_path
):
    import json, csv, os
...
def load_correctness_from_csv(username, eval, output_folder):
    ...
    # for a json file in correct_path = f"{get_content_path()}/kiva_{eval}"
    correct_path = f"{get_content_path()}/kiva_{eval}"
...
def update_analysis_results(
    analysis_results, data_dict, correctness_dict, answer_types
):
    """
    ...
    for img_id, correctness_list in correctness_dict.items():
        ...
        for i, answer_type in enumerate(answer_types):
            ...
            if transform_type not in answer_analysis["transform_type_results"]:
                answer_analysis["transform_type_results"][transform_type] = {
                    "total": 0,
                    "correct": 0,
                }
            answer_analysis["transform_type_results"][transform_type]["total"] += 1
            answer_analysis["transform_type_results"][transform_type]["correct"] += (
                is_correct_int
            )
...
def compute_accuracy_by_unique_trial(analysis_results):
    ...
    for answer_type, answer_analysis in analysis_results.items():
        for trial_id, trial_data in answer_analysis["trial_accuracies"].items():
            ...
            if (
                transform_type_and_subtype
                not in answer_analysis["transform_type_results"]
            ):
                answer_analysis["transform_type_results"][
                    transform_type_and_subtype
                ] = {"total": 0, "correct": 0, "trial_accuracies": []}
...
def aggregate_by_transformation_category(analysis_results):
    ...
    """
    for answer_type, answer_analysis in analysis_results.items():
        for transform_type_and_subtype, results in answer_analysis[
            "transform_type_results"
        ].items():
            if "trial_accuracies" in results:
                ...
                transformation_type = [
                    key
                    for key in answer_analysis["transform_type_aggregated"].keys()
                    if transform_type_and_subtype.startswith(key)
                ][0]
                answer_analysis["transform_type_aggregated"][
                    transformation_type
                ].extend(results["trial_accuracies"])
...
def plot_results(analysis_results):
    ...
    for answer_key, answer_analysis in analysis_results.items():
        ...
        for transform_type, accuracies in answer_analysis[
            "transform_type_aggregated"
        ].items():
            if accuracies:  # Only process if there is data
                # Relabel transformation types to a fixed concept name:
                if (
                    "colour" in transform_type.lower()
                    or "color" in transform_type.lower()
                ):
                    concept = "Color"
    ...
    filtered_order = [
        concept for concept in custom_order if concept in available_concepts
    ]
