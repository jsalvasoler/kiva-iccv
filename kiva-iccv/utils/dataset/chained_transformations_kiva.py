...
def parse_arguments():
    ...
    parser.add_argument(
        "--transformation_list",
        type=str,
        help="Comma-separated list of transformation:parameter pairs "
        '(e.g., "Colour:Red,2DRotation:+90")',
    )
    parser.add_argument(
...
base_transform = transforms.Compose([transforms.Resize((128, 128), antialias=True)])
...
for incompatible_set in incompatible_transformations:
    ...
    if count > 1:
        raise ValueError(
            f"The selected transformations may cancel each other out: "
            f"{', '.join([f'{t}:{p}' for t, p in incompatible_set])}"
        )
...
if duplicates:
    raise ValueError(
        f"Multiple parameter values are identified with more "
        f"than one transformation type(s) {duplicates}"
    )

num_transformations = len(transformations)
base_count = (
    trials // num_transformations
)  # Each transformation should be last at least this many times
remainder = trials % num_transformations

# Build a list with equal counts for each transformation
# (list of "last" transformations for each trial)
last_trans_list = []
...
def apply_counting(image, param, type="train", initial_count=None):
    ...
    if operation == "+":
        ...
        starting_count = (
            initial_count if initial_count is not None else random.choice(starting_options)
        )
...
def apply_rotation(image, angle, type="train"):
    if angle == "+90":
        correct_angle = -90
        incorrect_angle = 180
    elif angle == "-90":
...
def apply_transformation_chain(image, transformation, parameter, type="train", initial_value=None):
    if transformation == "Colour":
        if type == "train":
            initial_image, correct_image, input_val, output_val = apply_color(
        ...
        if type == "train":
            ...
            initial_image, correct_image, incorrect_image, input_val, incorrect_val = (
                apply_counting(image, parameter, type="test", initial_count=initial_value)
            )
            return initial_image, correct_image, incorrect_image, input_val, incorrect_val
...
while True:
    try:
        ...
        transformations = other_transformations + [
            designated_last
        ]  # Create final order with designated last transformation at end
        ...
        for transform_idx, (transform_name, parameter) in enumerate(transformations):
            if transform_name == "Counting":
                if transform_idx == 0:
                    current_train_output, current_test_output = (
                        current_train_input,
                        current_test_input,
                    )
            ...
            train_input, _, input_val, output_val = apply_transformation_chain(
                current_train_input, transform_name, parameter, type="train"
            )
            ...
            test_input, test_output, _, input_val, output_val = result = apply_transformation_chain(
                current_test_input, transform_name, parameter, type="test", initial_value=input_val
            )
        ...
        if count_index is not None:
            # Train input & output
            current_train_input, _, count_val, count_output_val = apply_counting(
                current_train_input.clone(), count_param, type="train"
            )

            _, current_train_output, _, _, _ = apply_counting(
                current_train_output.clone(), count_param, type="test", initial_count=count_val
            )

            # Test input, correct output, & incorrect output
            current_test_input, _, _, _, _ = apply_counting(
                current_test_input.clone(), count_param, type="test", initial_count=count_val
            )

            if count_index == (
                len(transformations) - 1
            ):  # If Counting is the random incorrect option
                _, _, current_test_incorrect, _, count_incorrect_val = apply_counting(
                    current_test_correct.clone(), count_param, type="test", initial_count=count_val
                )
                input_val = count_val
                train_output_value = count_output_val
                incorrect_value = count_incorrect_val
            else:
                _, current_test_incorrect, _, _, count_incorrect_val = apply_counting(
                    current_test_incorrect.clone(),
                    count_param,
                    type="test",
                    initial_count=count_val,
                )

            _, current_test_correct, _, _, _ = apply_counting(
                current_test_correct.clone(), count_param, type="test", initial_count=count_val
            )
