...
def parse_arguments():
    ...
    parser.add_argument(
        "--input_directory", type=str, help="Path to the input directory containing the images."
    )
    parser.add_argument(
        "--output_directory",
        type=str,
    ...
    parser.add_argument(
        "--next_index", type=int, default=0, help="The starting index for naming the output files."
    )
    parser.add_argument("--trials", type=int, default=1, help="The number of trials to run.")
    parser.add_argument(
        "--transformation_list",
        type=str,
        help="Comma-separated list of 2 transformation:parameter pairs "
        '(e.g., "Counting:+1,2DRotation:+90")',
    )
    parser.add_argument(
        "--shuffle", type=bool, default=False, help="To shuffle the objects dataset."
    )
...
def order_counting_last(transformation_list):
    ...
    if len(transformation_list) == 2 and transformation_list[0][0] == "Counting":
        transformation_list[0], transformation_list[1] = (
            transformation_list[1],
            transformation_list[0],
        )
...
if "Resize" not in [t[0] for t in transformations]:
    ...
    base_transform = transforms.Compose([transforms.Resize((600, 600), antialias=True)])
else:
    base_transform = transforms.Compose([transforms.Resize((300, 300), antialias=True)])
...
# Incompatibilities: 1) Support only 2 transformation pairs,
# 2) Reflect & 2DRotation are incompatible
if len(transformations) > 2:
    raise ValueError(
        "We currently only support 2 transformation pairs. "
        "Please limit your input to 2 transformation pairs."
    )
...
if duplicates:
    raise ValueError(
        f"Multiple parameter values are identified with one transformation type: {duplicates}."
    )
...
def save_values_to_txt(
    index, initial_train, output_train, initial_test, correct_test, incorrect_test0, incorrect_test1
):
    """
    Args:
...
while True:
    try:
        ...
        for transform_idx, (transform_name, parameter) in enumerate(transformations):
            if transform_idx == 0:
                train_input, train_output, input_value, output_value = apply_transformation_chain(
                    current_train_input, transform_name, parameter, "train"
                )
            # For Counting and 2DRotation, ensure we keep the intial starting
            # count / rotation the same
            # We don't need to do this for Reflect and Resize
            # because they don't have an initial transformation
            elif transform_idx == 1:
                train_input, _, input_value, output_value = apply_transformation_chain(
                    current_train_input, transform_name, parameter, "train"
                )

                # Use the value from the first transformation if available
                if current_train_output is not None:
                    _, train_output, _, _ = apply_transformation_chain(
                        current_train_output,
                        transform_name,
                        parameter,
                        "train",
                        keep_initial_value=input_value,
                    )  # Keep initial value to ensure test inputs don't start with same
                else:
                    # Fallback if current_train_output is not available
        ...
        # ——————————————————————————————————————————————————————————————————————————————
        # TEST STIMULI: 1) test_input, 2) test_correct_output, 3) test_incorrect_output0,
        # 4) test_incorrect_output1
        # ——————————————————————————————————————————————————————————————————————————————
        # Initialize test variables
        current_test_correct = None
        current_test_incorrect0 = None

        for transform_idx, (transform_name, parameter) in enumerate(transformations):
            # Apply transformation to 'train_input' and 'train_output'
            if transform_idx == 0:
                test_input, test_output, test_incorrect, input_value, incorrect_value0 = (
                    apply_transformation_chain(
                        current_test_input,
                        transform_name,
                        parameter,
                        "test",
                        avoid_initial_value=train_input_txt[0][0],
                    )
                )
            # For Counting and 2DRotation, ensure we keep the intial starting
            # count / rotation the same
            ...
            # because they don't have an initial transformation
                ...
                test_input, _, _, input_value, _ = apply_transformation_chain(
                    ...
                    parameter,
                    "test",
                    avoid_initial_value=train_input_txt[1][0],
                )  # Avoid initial value to ensure test inputs don't start with same as train

                # Use the values from the first transformation if available
                if current_test_correct is not None:
                    _, test_output, test_incorrect_output1, _, incorrect_value1 = (
                        apply_transformation_chain(
                    ...
                    # Fallback if current_test_correct is not available
                    test_output = test_input.clone()
                    test_incorrect_output1 = test_input.clone()
                    incorrect_value1 = "0"

                # Make incorrect A and correct B
                if current_test_incorrect0 is not None:
                    ...
                    # Fallback if current_test_incorrect0 is not available
                    test_incorrect_output0 = test_input.clone()
            ...
            if not (transform_name == "Counting" or transform_name == "2DRotation"):
                ...
                if transform_idx == 0:
                    ...
                    test_incorrect_output1_txt = [
                        test_correct_output_txt[0],
                        (transformations[1][0], incorrect_value1[1]),
                    ]
                    test_incorrect_output0_txt = [
                        (transformations[0][0], incorrect_value0),
                        test_correct_output_txt[1],
                    ]
