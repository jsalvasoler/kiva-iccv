...
def setup_kiva_data_set(name, base_data_path):
    ...
    run_command(f"unzip -qo {download_zip_location} -d {target_img_dir}")
    ...
    if os.path.isdir(nested_path) and os.listdir(nested_path):
        run_command(f"mv {nested_path}/* {target_img_dir}/")
        run_command(f"rmdir {nested_path}")  # Remove the now empty nested folder

    # 5. REMOVE: macOS metadata directories and files
    macos_dir = os.path.join(target_img_dir, "__MACOSX")
    if os.path.exists(macos_dir):
        run_command(f"rm -rf {macos_dir}")

    # Also remove any stray ._ files directly in target_img_dir or its subdirectories
    run_command(f'find {target_img_dir} -name "._*" -delete')

    # 6. Remove the original zip file
    run_command(f"rm {download_zip_location}")
    ...
    class RadarAxes(PolarAxes):
        ...
        def _gen_axes_patch(self):
            if frame == "circle":
                ...
                return patches.RegularPolygon((0.5, 0.5), num_vars, radius=0.5, edgecolor="k")
            else:
...
def radar_plot_pt(
    ...
    fig, axs = plt.subplots(figsize=(8, 8), nrows=1, ncols=1, subplot_kw={"projection": "radar"})
...
def init_analysis_results(answer_types, transform_types):
    ...
    return {
        answer_type: {
            ...
            "trial_accuracies": {},
            "transform_type_aggregated": {transform: [] for transform in transform_types},
        }
...
def compute_accuracy_by_unique_trial(analysis_results):
    ...
    """
    for _, answer_analysis in analysis_results.items():
        for trial_id, trial_data in answer_analysis["trial_accuracies"].items():
            ...
            if (
                "trial_accuracies"
                not in answer_analysis["transform_type_results"][transform_type_and_subtype]
            ):
                answer_analysis["transform_type_results"][transform_type_and_subtype][
                    "trial_accuracies"
                ] = []
    ...
    for _, answer_analysis in analysis_results.items():
        for transform_type_and_subtype, results in answer_analysis[
            ...
            if "trial_accuracies" in results:
                ...
                answer_analysis["transform_type_aggregated"][transformation_type].extend(
                    results["trial_accuracies"]
                )
